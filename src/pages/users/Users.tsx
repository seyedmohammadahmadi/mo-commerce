...
interface User {
  id: string | number; // در db.json ممکنه رشته باشد
  username: string;
  role: "ADMIN" | "CUSTOMER";
}
...
function Users() {
  ...
  const handleSubmit = async () => {
    try {
      if (!isDev) {
        toast.warning("Demo is read-only on GitHub Pages.");
        setOpen(false);
        return;
      }
      if (formData.id) {
                ...
                <TableRow key={String(user.id)}>
                  ...
                  <TableCell className="text-right space-x-2">
                    <Button
                      ...
                      onClick={() => {
                        setFormData({
                          // در دمو (prod) id ممکن است استرینگ باشد و ویرایش غیرفعال است
                          id:
